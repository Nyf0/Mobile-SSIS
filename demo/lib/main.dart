      ...
      body: SingleChildScrollView(
        child: Center(
          child: Column(
            children: [
              SizedBox(height: 20),
              Text('Add / Update / Delete Course', style: Theme.of(context).textTheme.headline6),
              SizedBox(height: 20),
              TextField(
                ...
                onChanged: (value) {
                  if (value.isNotEmpty) {
                    // Fetch course details based on the entered ID
                    fetchCourse(int.tryParse(value) ?? 0);
                  } else {
                    // Clear course name if the ID field is empty
                    clearCourseDetails();
                  }
                },
              ...
              Row(
                ...
                children: [
                  ...
                  ElevatedButton(
                    onPressed: () {
                      deleteCourse(int.tryParse(courseIdController.text) ?? 0);
                    },
                    child: const Text('Delete'),
                  ),
                  ElevatedButton(
                    onPressed: () {
                      updateCourse(courseNameController.text, int.tryParse(courseIdController.text) ?? 0);
                    },
                    child: const Text('Update'),
                  ),
                  ElevatedButton(
                    onPressed: () {
                      searchCourse(int.tryParse(courseIdController.text) ?? 0);
                    },
                  ...
                  ElevatedButton(
                    ...
                    child: const Text('Refresh'), // Add refresh button
                  ),
                ],
              ),
              SizedBox(height: 20),
      ...
      body: SingleChildScrollView(
        child: Center(
          child: Column(
            children: [
              SizedBox(height: 20),
              Text('Add / Update / Delete Student', style: Theme.of(context).textTheme.headline6),
              SizedBox(height: 20),
              TextField(
                ...
                onChanged: (value) {
                  if (value.isNotEmpty) {
                    ...
                    // Clear student name and course fields if the ID field is empty
                    clearStudentDetails();
                  }
                },
                decoration: const InputDecoration(
              ...
              SizedBox(height: 10),
              // Display radio buttons for selecting a course
              Column(
                ...
                children: [
                  ...
                  SizedBox(height: 10),
                  // Generate radio buttons for each course
                  ...courses.map<Widget>((course) {
                    return Row(
                      children: [
                        Radio<int>(
                          value: course['course_id'], // Use course ID as the value
                        ...
                        Text('${course['course_name']}'), // Display course name
                      ],
                    );
                  }).toList(),
                ],
              ...
              Row(
                ...
                children: [
                  ElevatedButton(
                    onPressed: () {
                      addStudent(studentNameController.text, selectedCourseId ?? 0);
                    },
                    child: const Text('Add'),
                  ),
                  ElevatedButton(
                    onPressed: () {
                      deleteStudent(int.tryParse(studentIdController.text) ?? 0);
                    },
                    child: const Text('Delete'),
                  ),
                  ElevatedButton(
                    onPressed: () {
                      updateStudent(
                        int.tryParse(studentIdController.text) ?? 0, // Convert to int
                  ...
                  ElevatedButton(
                    onPressed: () {
                      searchStudent(int.tryParse(studentIdController.text) ?? 0);// Implement search functionality
                    },
                    child: const Text('Search'),
                  ),
                  ElevatedButton(
